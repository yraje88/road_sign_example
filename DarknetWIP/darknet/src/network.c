...
void forward_network_partial(network *netp, int start, int finish)
{
    network net = *netp;
    int i;
    for(i = start; i <= finish; ++i){
        net.index = i;
        layer l = net.layers[i];
        // printf("Forward layer of type: %d\n", l.type);
        if(l.delta){
            fill_cpu(l.outputs * l.batch, 0, l.delta, 1);
        }
        l.forward(l, net);
...
void forward_network_merge(network *netp, int start, int finish)
{
    network net = *netp;
    int i;
    for(i = start; i <= finish; ++i){
...
void update_network_partial(network *netp, int start, int finish)
{
#ifdef GPU
    if(netp->gpu_index >= 0){
        update_network_gpu(netp);   
...
void backward_network_partial(network *netp, int start, int finish)
{
#ifdef GPU
    if(netp->gpu_index >= 0){
        backward_network_gpu(netp);   
    ...
    for(i = start; i >= finish; --i){
        layer l = net.layers[i];
        // printf("Backward layer of type: %d\n", l.type);
        if(l.stopbackward) break;
        if(i == 0){
            net = orig;
        }else{
...
float *network_predict_partial(network *net, float *input, int start, int finish)
{
    network orig = *net;
    net->input = input;
    net->truth = 0;
    ...
    net->delta = 0;
    printf("Here\n");
    forward_network_partial(net, start, finish);
    printf("here\n");
    float *out = net->output;
    *net = orig;
    return out;
