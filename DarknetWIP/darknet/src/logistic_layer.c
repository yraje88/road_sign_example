...
layer make_logistic_layer(int batch, int inputs)
{
    ...
    l.forward_merge = forward_merge_logistic_layer;
    #ifdef GPU
...
void forward_merge_logistic_layer(const layer l, network net)
{
    // printf("Going forward\n");
    copy_cpu(l.outputs*l.batch, net.input, 1, l.output, 1);
    activate_array(l.output, l.outputs*l.batch, LOGISTIC);
    if(net.truth){
        // printf("here\n");
        logistic_x_ent_merge(l.batch*l.inputs, l.output, net.truth, l.delta, l.loss);
        l.cost[0] = sum_array(l.loss, l.batch*l.inputs);
        // printf("cost: %f\n", l.cost[0]);
    }
}
